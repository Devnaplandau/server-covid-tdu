...
exports.summary = async (req, res) => {
  try {
    ...
    // count total vaccine dose "lieu luong vaccine"
    ...
    // count total used vaccine does
    const totalVaccineDoesUsed = await VaccineLot.aggregate().group({
      _id: null,
    ...
    // count user who has one vaccine does
    ...
    res.status(200).json({
      ...
      availableVaccineDoes:
        (totalVaccineDose[0] ? totalVaccineDose[0].quantity : 0) -
        (totalVaccineDoesUsed[0] ? totalVaccineDoesUsed[0].vaccinated : 0),
      latestVaccineLot,
